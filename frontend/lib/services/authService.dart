...
Future<void> _saveAuthData(String token, String role) async {
  ...
  Future<AuthResponse> _handleAuthRequest(Uri url, Map<String, dynamic> body) async {
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        final errorMessage = errorBody['message'] ?? errorBody['error'] ?? 'Erreur de connexion. Code: ${response.statusCode}';
        return AuthResponse(error: errorMessage);
      }
    } catch (e) {
      print('Erreur réseau/parsing: $e');
      return AuthResponse(error: 'Impossible de contacter le serveur. Le réseau a échoué.');
    }
  }
